...
INLINE static void push(mp_limb i, struct is **sp)
{
    struct is *tmp;
    tmp = *sp;
...
INLINE static mp_limb pop(struct is **sp)
{
    struct is *tmp;
    mp_limb i;
    if (!(*sp))
...
void mpz_init_set(MP_INT *s, const MP_INT *t)
{
    int i;
...
void mpz_init_set_ui(MP_INT *s, long unsigned int v)
{
    s->p = (mp_limb *) malloc(sizeof(mp_limb) * 2);
    if (!(s->p))
	fatal("mpz_init: cannot allocate memory");
...
void mpz_init_set_si(MP_INT *y, long int v)
{
...
static int lzb(mp_limb a)
{
    mp_limb i;
    int j;
    j = 0;
    for (i = ((mp_limb) 1 << (DIGITBITS - 1)); i && !(a & i);
	 j++, i >>= 1);
    return j;
}

void _mpz_realloc(MP_INT *x, mp_size size)
{
    int i;
    if (size > 1 && x->sz < size) {
#ifdef DEBUG
	printf("realloc %08lx to size = %ld ", (long) (x->p),
	       (long) (size));
#endif
...
void dgset(MP_INT *x, unsigned int i, mp_limb n)
{
    if (n) {
	if (i >= x->sz) {
...
static unsigned int digits(const MP_INT *x)
{
    int i;
...
/* y = x */
void mpz_set(MP_INT *y, const MP_INT *x)
{
...
void mpz_set_ui(MP_INT *y, long unsigned int v)
{
    int i;
    for (i = 1; i < y->sz; i++)
	y->p[i] = 0;
...
unsigned long mpz_get_ui(MP_INT *y)
{
    return (y->p[0] | (y->p[1] << DIGITBITS));
}

long mpz_get_si(MP_INT *y)
{
    if (y->sn == 0)
	return 0;
    else
...
void mpz_set_si(MP_INT *y, long int v)
{
    int i;
    for (i = 1; i < y->sz; i++)
	y->p[i] = 0;
...
static void uadd(MP_INT *z, const MP_INT *x, const MP_INT *y)
{
    mp_limb c;
    int i;
    const MP_INT *t;
...
static void usub(MP_INT *z, const MP_INT *y, const MP_INT *x)
{
    int i;
    mp_limb b, m;
...
static int ucmp(const MP_INT *y, const MP_INT *x)
{
    int i;
...
int mpz_cmp(const MP_INT *x, const MP_INT *y)
{
    int abscmp;
    if (x->sn < 0 && y->sn > 0)
	return (-1);
...
/* is abs(x) == 0 */
static int uzero(const MP_INT *x)
{
    int i;
    for (i = 0; i < x->sz; i++)
	if ((x->p)[i] != 0)
...
/* w = u * v */
void mpz_mul(MP_INT *ww, const MP_INT *u, const MP_INT *v)
{
    int i, j;
    mp_limb t0, t1, t2, t3;
    mp_limb cc;
	...
	for (i = 0; i < 2 * v->sz; i++) {
		...
		w->p[(i + j) / 2] =
		    (HIGH(w->p[(i + j) / 2]) << HALFDIGITBITS) | t2;
...
/* n must be < DIGITBITS */
static void urshift(MP_INT *c1, MP_INT *a, int n)
{
    mp_limb cc = 0;
    if (n >= DIGITBITS)
...
/* n must be < DIGITBITS */
static void ulshift(MP_INT *c1, const MP_INT *a, int n)
{
    mp_limb cc = 0;
    if (n >= DIGITBITS)
...
void mpz_div_2exp(MP_INT *z, MP_INT *x, long unsigned int e)
{
    short sn = x->sn;
    if (e == 0)
	mpz_set(z, x);
...
void mpz_mul_2exp(MP_INT *z, MP_INT *x, long unsigned int e)
{
    short sn = x->sn;
    if (e == 0)
	mpz_set(z, x);
...
void mpz_mod_2exp(MP_INT *z, MP_INT *x, long unsigned int e)
{
    short sn = x->sn;
    if (e == 0)
	mpz_set_ui(z, 0L);
...
static void udiv(MP_INT *qq, MP_INT *rr, const MP_INT *xx, const MP_INT *yy)
{
    MP_INT *q, *x, *y, *r;
...
void mpz_add(MP_INT *zz, const MP_INT *x, const MP_INT *y)
{
    int mg;
    MP_INT *z;
    if (x->sn == 0) {
...
void mpz_add_ui(MP_INT *x, const MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_add(x, y, &z);
...
int mpz_cmp_ui(const MP_INT *x, long unsigned int n)
{
    MP_INT z;
    int ret;
    mpz_init_set_ui(&z, n);
...
int mpz_cmp_si(const MP_INT *x, long int n)
{
    MP_INT z;
    int ret;
    mpz_init(&z);
...
void mpz_mul_ui(MP_INT *x, const MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_mul(x, y, &z);
...
void mpz_mul_si(MP_INT *x, const MP_INT *y, long int n)
{
    MP_INT z;
    mpz_init_set_si(&z, n);
    mpz_mul(x, y, &z);
...
void mpz_sub( MP_INT *z, const MP_INT *x, const MP_INT *y)
{
    MP_INT u;
    mpz_init(&u);
    mpz_set(&u, y);
...
void mpz_sub_ui(MP_INT *x, const MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_sub(x, y, &z);
...
void mpz_div(MP_INT *q, const MP_INT *x, const MP_INT *y)
{
    MP_INT r;
...
void mpz_mdiv(MP_INT *q, const MP_INT *x, const MP_INT *y)
{
    MP_INT r;
...
void mpz_mod(MP_INT *r, const MP_INT *x, const MP_INT *y)
{
    MP_INT q;
    short sn = x->sn;
    mpz_init(&q);
...
void mpz_divmod(MP_INT *q, MP_INT *r, const MP_INT *x, const MP_INT *y)
{
    short sn1 = x->sn, sn2 = y->sn;
    if (x->sn == 0) {
	zero(q);
...
void mpz_mmod(MP_INT *r, MP_INT *x, MP_INT *y)
{
    MP_INT q;
    short sn1 = x->sn, sn2 = y->sn;
    mpz_init(&q);
...
void mpz_mdivmod(MP_INT *q, MP_INT *r, const MP_INT *x, const MP_INT *y)
{
    short sn1 = x->sn, sn2 = y->sn, qsign;
    if (sn1 == 0) {
	zero(q);
...
void mpz_mod_ui(MP_INT *x, MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init(&z);
    mpz_set_ui(&z, n);
...
void mpz_mmod_ui(MP_INT *x, MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init(&z);
    mpz_set_ui(&z, n);
...
void mpz_div_ui(MP_INT *x, const MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_div(x, y, &z);
...
void mpz_mdiv_ui(MP_INT *x, const MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_mdiv(x, y, &z);
...
void mpz_divmod_ui(MP_INT *q, MP_INT *x, const MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_divmod(q, x, y, &z);
...
void mpz_mdivmod_ui(MP_INT *q, MP_INT *x, MP_INT *y, long unsigned int n)
{
    MP_INT z;
    mpz_init_set_ui(&z, n);
    mpz_mdivmod(q, x, y, &z);
...
unsigned int mpz_sizeinbase(const MP_INT *x, int base)
{
    int i, j;
...
char *mpz_get_str(char *s, int base, const MP_INT *x)
{
    MP_INT xx, q, r, bb;
    char *p, *t, *ps;
    ...
    mp_limb d;
    if (base < 2 || base > 36)
	fatal("base out of range 2..36 in mpz_get_str");
    t = (char *) malloc(sz + 2);
    if (!t)
...
int mpz_set_str(MP_INT *x, const char *s, int base)
{
    int l, i;
    int retval = 0;
    MP_INT t, m, bb;
...
int mpz_init_set_str(MP_INT *x, const char *s, int base)
{
    mpz_init(x);
    return mpz_set_str(x, s, base);
}

#define mpz_randombyte (rand()& 0xff)

void mpz_random(MP_INT *x, unsigned int size)
{
...
void mpz_random2(MP_INT *x, unsigned int size)
{
    unsigned int bits = size * LONGBITS;
    unsigned int digits = bits / DIGITBITS;
    unsigned int oflow = bits % DIGITBITS;
...
size_t mpz_size(const MP_INT *x)
{
...
void mpz_abs(MP_INT *x, MP_INT *y)
{
    if (x != y)
	mpz_set(x, y);
    if (uzero(x))
...
void mpz_neg(MP_INT *x, const MP_INT *y)
{
    if (x != y)
	mpz_set(x, y);
    x->sn = -(y->sn);
...
void mpz_fac_ui(MP_INT *x, long unsigned int n)
{
    mpz_set_ui(x, 1L);
    if (n == 0 || n == 1)
	return;
...
void mpz_gcd(MP_INT *gg, MP_INT *aa, MP_INT *bb)
{
    MP_INT *g, *t, *a, *b;
...
void mpz_gcdext(MP_INT *gg, MP_INT *xx, MP_INT *yy, MP_INT *aa, MP_INT *bb)
{
    MP_INT *x, *y, *g, *u, *q;
...
int mpz_jacobi(MP_INT *ac, MP_INT *bc)
{
    int sgn = 1;
    unsigned long c;
    MP_INT *t, *a, *b;
...
void mpz_and(MP_INT *z, MP_INT *x, MP_INT *y)		/* not the most efficient way to do this */
...
void mpz_or(MP_INT *z, MP_INT *x, MP_INT *y)		/* not the most efficient way to do this */
...
void mpz_xor(MP_INT *z, MP_INT *x, MP_INT *y)		/* not the most efficient way to do this */
...
void mpz_pow_ui(MP_INT *zz, MP_INT *x, long unsigned int e)
{
    MP_INT *t;
...
void mpz_powm(MP_INT *zz, MP_INT *x, MP_INT *ee, MP_INT *n)
{
    MP_INT *t, *e;
...
void mpz_powm_ui(MP_INT *z, MP_INT *x, long unsigned int e, MP_INT *n)
{
    MP_INT f;
    mpz_init(&f);
    mpz_set_ui(&f, e);
...
#if 0
/* Miller-Rabin */
static int witness(MP_INT *x, MP_INT *n)
{
    MP_INT *t, *e, *e1;
    struct is *stack = NULL;
    int trivflag = 0;
...
int mpz_probab_prime_p(MP_INT *nn, int s)
{
    MP_INT *a, *n;
    int j, k, i;
    long t;
 ...
 */
void mpz_sqrtrem(MP_INT *root, MP_INT *rem, MP_INT *a)
{
    MP_INT tmp;
    MP_INT r;
    mp_limb z;
...
void mpz_sqrt(MP_INT *root, MP_INT *a)
{
...
/*
 * ADDITIONS -- PerPet 2003
 */

/* Computes inv = u^(-1) mod v */
int mpz_invert(mpz_t inv, const mpz_t u, const mpz_t v)
{
    /* Origin: Knuth Vol 2 p 342
...
TODO: if not exist???
    ...
    /* initialize */
    mpz_init_set_ui(u1, 1);	/* u1 = 1 */
    mpz_init_set(u3, u);	/* u3 = u */
    mpz_init_set_ui(v1, 0);	/* v1 = 0 */
    mpz_init_set(v3, v);	/* v3 = v */
    mpz_init(t1);
    mpz_init(t3);
    mpz_init(q);
    mpz_init(w);

    iter = 0;			/* odd/even iterations */
    while (mpz_cmp_si(v3, 0) != 0) {	/* Loop while v3 != 0 */

	/* Step X3. Divide and "Subtract" */
	mpz_divmod(q, t3, u3, v3);	/* q = u3 / v3, t3 = u3 % v3 */
	mpz_mul(w, q, v1);	/* w = q * v1 */
	mpz_add(t1, u1, w);	/* t1 = u1 + w */
    ...
    if (iter)		/* odd number of iterations */
	mpz_sub(inv, v, u1);	/* inv = v - u1 */
    else
	mpz_set(inv, u1);	/* inv = u1 */
...
int mpz_tstbit(const mpz_t n, unsigned long i)
{
	...
	return 0;
    p = n->p + (i / DIGITBITS);
    return ((*p) & (1L << (i % DIGITBITS))) != 0;
}


int mpz_out_str(FILE*file, int base, const mpz_t x) {
    int sz;
    char *s;
    if(file==NULL)
        file = stdout;
    sz = mpz_sizeinbase(x, base);
    s = malloc(sz+1);
    if(!s)
	fatal("cannot allocate memory in mpz_out_str");
...
#if 0
/* little endian bytes */
int mpz_set_mem(MP_INT *x, const char *s, int len)
{
    int i;
    MP_INT t, m;
    ...
    if (len < 0)
	fatal("mpz_set_mem: invalid length");

    for (i = 0; i < len; i++) {
	mpz_mul_ui(&t, &m, (unsigned char) s[i]);
	mpz_add(x, x, &t);
	mpz_mul_2exp(&m, &m, 8);
    }
    /* POSTCONDITION: always positive */
    mpz_clear(&m);
    mpz_clear(&t);
    return 0;
}

int mpz_get_mem(char *s, int len, const MP_INT *x)
{
    int sz = mpz_sizeinbase(x,2); /* size in bits */
    int i;

    memset(s,0,len);

    while(sz>0 && mpz_tstbit(x,sz-1)==0) 
        sz--;

    if (len*8 < sz)
	fatal("len out of range in mpz_get_mem");

    if (mpz_cmp_si(x,0)) /* is zero */
	return 0;

    if (x->sn < 0) {
        fatal("can't convert negative number in mpz_get_mem");
    }

    for(i=0; i<len; i++) {
       int j;
       int b=0;
       for(j=0;j<8;j++) { /* get byte */
           if(mpz_tstbit(x,i*8+j)==1) b != (1<<j);
       } 
       *s++ = b; /* store byte */
    }
    return 0;
